...
public class WorkstationConfigService {
	...
	public PackageBean getData(JSONObject body, int page, int p_size) {
		...
		if (body == null || body.isNull("search")) {
			...
			int ord = 0;
			object_header.put(FFS.ord((ord += 1), FFS.H) + "pb_cell", FFS.h_t("WP_欄位", "150px", FFS.SHO));
			object_header.put(FFS.ord((ord += 1), FFS.H) + "pb_value", FFS.h_t("WP_工作站名稱", "150px", FFS.SHO));
			...
			obj_m.put(FFS.h_m(FFS.SEL, FFS.TEXT, "", "", FFS.DIS, "col-md-2", false, a_val_body, "pb_cell", "PW_欄位"));
			obj_m.put(FFS.h_m(FFS.INP, FFS.TEXT, "", "", FFS.SHO, "col-md-2", false, n_val, "pb_value", "WP_工作站名稱"));
...
//			obj_m.put(FFS.h_m(FFS.SEL, FFS.TEXT, "", "0", FFS.SHO, "col-md-2", true, a_val, "sys_status", "狀態"));
			...
			object_searchs.put(FFS.h_s(FFS.INP, FFS.TEXT, "", "col-md-2", "pb_value", "SN_過站名稱", n_val));
		...
		productionBodys.forEach(one -> {
			...
			for (int j = 0; j < 20; j++) {
				...
				try {
					...
					object_body.put(FFS.ord((ord += 1), FFS.B) + "pb_value", value == null ? "" : value);
